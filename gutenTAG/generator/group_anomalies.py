...
def apply_group_anomaly(
    *,
    ...
    if any(
        hasattr(bo, "_process_state") for bo in channel_bos
    ) and anomaly_type not in {"covariance-change", "correlation-flip"}:
        raise ValueError(
            "Unsupported white-noise LMC operator; use the explicit v13 process-law API, not an observed-window fallback"
        )
