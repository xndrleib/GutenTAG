...
def apply_lmc_noise_correlation(
    *, channel_bos: Sequence[Any], seed: int, config: Mapping[str, Any]
) -> bool:
    ...
    b, corr = sample_lmc_correlation(
        c,
        rng,
        rank=int(config.get("lmc_rank", min(3, c))),
        sparsity=float(config.get("lmc_sparsity", 0.25)),
        idiosyncratic=kappa,
    )
    d = np.sqrt(np.sum(b * b, axis=1) + kappa)
    z = (
        rng.normal(size=(n, b.shape[1])) @ b.T
        + np.sqrt(kappa) * rng.normal(size=(n, c))
    ) / d
    scales = np.asarray(
        [
            (
                abs(float(bo.variance) * float(bo.amplitude))
                if hasattr(bo, "variance") and hasattr(bo, "amplitude")
                else float(np.std(x))
            )
            for bo, x in zip(channel_bos, noises)
        ]
    )
    state = NoiseLawState(
        readonly(corr), readonly(z), readonly(scales), readonly(b / d[:, None])
    )
...
def recouple_lmc_target(
    *,
    ...
    residual = (state.standardized[start:end, target] - rho0 * anchor_z) / np.sqrt(
        1 - rho0 * rho0
    )
    g = np.ones(end - start)
