...
class SplitSpec(BaseModel):
    ...
    role: Literal["train", "validation", "calibration", "iid", "parameter_ood", "family_ood", "mechanism_ood", "relation_only"]
    ...
    def validate_prior(self) -> SplitSpec:
        if not self.families or set(self.families)-{"diagonal-ar", "gp-lmc", "independent-gp"}:
        ...
        if not self.channels or min(self.channels) < 1 or len(set(self.channels)) != len(self.channels):
        ...
        if set(self.ood_parameters)-{"length_scale", "memory", "noise_std", "channels"}:
            raise ValueError("unknown OOD parameter")
        if self.role == "relation_only":
            if set(self.families) != {"diagonal-ar"} or set(self.mechanisms)-set(RELATION_ONLY+("null",)) or min(self.channels) < 2:
                raise ValueError("relation_only requires diagonal-ar covariance/precision/null and >=2 channels")
        return self
...
class BenchmarkConfig(BaseModel):
    ...
    def validate_contracts(self) -> BenchmarkConfig:
        if not self.splits or any(not re.fullmatch(r"[A-Za-z0-9_-]+", s) for s in self.splits):
        ...
        if self.event_fraction[0] <= 0 or self.event_fraction[1] > 0.8 or self.transition_fraction[1] >= 0.5:
            raise ValueError("events must leave normal context and nonempty interiors")
        if not self.window_lengths or min(self.window_lengths) < 2 or max(self.window_lengths) > min(self.length, self.reference_length):
            raise ValueError("window lengths must fit both reference and query")
        if not self.alpha_grid or any(not math.isfinite(a) or not 0 < a < 1 for a in self.alpha_grid):
            raise ValueError("invalid alpha grid")
        roles = {s.role for s in self.splits.values()}
        if not {"train", "validation", "calibration"} <= roles:
            raise ValueError("explicit train, validation and calibration roles required")
        ...
        for s in self.splits.values():
            if s.role == "iid":
                keys = ("families", "mechanisms", "channels", "length_scale", "memory", "noise_std")
                if not any(all(getattr(s, k) == getattr(t, k) for k in keys) for t in train):
                    raise ValueError("IID split must use a training prior, not merely a new name")
            ...
            if s.role == "parameter_ood":
                if not s.ood_parameters:
                    raise ValueError("parameter_ood requires declared disjoint dimensions")
                for t in train:
                    for key in s.ood_parameters:
                        if key == "channels":
                            overlap = bool(set(s.channels)&set(t.channels))
                        else:
                            lo, hi = getattr(s, key); a, b = getattr(t, key)
                ...
                if "noise_std" in s.ood_parameters and "diagonal-ar" in s.families:
                    raise ValueError("noise_std OOD is not a diagonal-ar prior parameter")
                ...
                if "length_scale" in s.ood_parameters and "diagonal-ar" in s.families:
                    raise ValueError("length_scale OOD is not meaningful for diagonal-ar")
        return self
