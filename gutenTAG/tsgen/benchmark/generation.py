...
def generate_benchmark(config: BenchmarkConfig, output: Path, *, overwrite: bool = False) -> dict[str, Any]:
    ...
    if output.exists():
        ...
        if not overwrite or not marker.is_file() or marker.read_text() != "synthgen.v13.1\n":
...
def _generate(config: BenchmarkConfig, root: Path) -> dict[str, Any]:
    ...
    for split, spec in config.splits.items():
        LOGGER.info("Generating split=%s role=%s systems=%d", split, spec.role, spec.systems)
        for index in tqdm(range(spec.systems), desc=split, leave=False):
            ...
            params = {"length_scale": float(np.exp(rng.uniform(*np.log(spec.length_scale)))),
                      "memory": float(rng.uniform(*spec.memory)),
                      "noise_std": float(rng.uniform(*spec.noise_std))}
            law = sample_law(family, channels, law_seed, rank=config.rank,
                             features=config.features, **params)
            ...
            for k in range(config.calibration_realizations):
                ...
                np.savez_compressed(folder / "calibration" / f"{k:05d}.npz", values=normal.values)
            plan_rng = np.random.default_rng(derive_seed(law_seed, "plan"))
            length = max(4, int(config.length*plan_rng.uniform(*config.event_fraction)))
            start = int(plan_rng.integers(1, config.length-length))
            transition = min((length-1)//2, int(length*plan_rng.uniform(*config.transition_fraction)))
            ...
            if spec.role != "calibration":
                for mechanism in spec.mechanisms:
                    if not compatible(family, mechanism, channels):
                        skipped.append({"split": split, "family": family, "mechanism": mechanism,
                                        "reason": "unsupported law/operator contract"})
                        continue
                    signed = strength
                    if mechanism not in RELATION_ONLY + SENSOR_KINDS and plan_rng.random() < 0.5:
                        signed = -signed
                    event = Intervention(mechanism, start, start+length, 0.0 if mechanism == "null" else signed,
                                         (channel,), transition)
                    genotype = hashlib.sha256(json.dumps({"law_id": law.law_id, **vars(event)}, sort_keys=True).encode()).hexdigest()
                    for replica in range(config.replicas_per_genotype):
                        ...
                        changed = apply_interventions(clean, [event], seed=derive_seed(seed, "observation"))
                        key = f"{mechanism}-{replica:05d}"
                        np.savez_compressed(folder / "queries" / f"{key}.npz",
                                            values=changed.values, observed_mask=changed.observed_mask,
                                            timestamps=changed.timestamps)
                        np.savez_compressed(folder / "oracle" / f"{key}.npz", clean=clean.values,
                                            intervention_mask=changed.intervention_mask,
                                            effect_mask=changed.effect_mask, evaluation_mask=changed.evaluation_mask)
                        write_json(folder / "oracle" / f"{key}.json", changed.events)
                        query_records.append({"id": key, "realization_id": clean.realization_id,
                                              "genotype_id": genotype, "seed": seed,
                                              "mechanism": mechanism, "replica": replica})
            record = {"system_id": system_id, "law_id": law.law_id, "split": split,
                      "role": spec.role, "family": family, "channels": channels,
                      "parameters": params, "law_seed": law_seed, "reference_seed": ref_seed,
                      "calibration_seeds": calibration_seeds, "queries": query_records,
                      "relative_path": str(folder.relative_to(root))}
            all_seeds = [ref_seed, *calibration_seeds, *(q["seed"] for q in query_records)]
    ...
    manifest = {"schema_version": config.schema_version, "config": config.model_dump(mode="json"),
                "systems": systems, "skipped_combinations": skipped, "sha256": hashes,
    ...
    LOGGER.info("Generation complete: systems=%d queries=%d", len(systems), sum(len(s["queries"]) for s in systems))
    return manifest
