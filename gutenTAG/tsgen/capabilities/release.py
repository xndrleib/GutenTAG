...
def build_generator_feedback(analysis_dir: Path) -> dict[str, Any]:
    ...
    reason_counts = _reason_counts(events, ("failure_reasons", "warning_reasons", "provisional_gate_hits"))
    ...
    return {
        ...
        "top_reason_counts": dict(sorted(reason_counts.items(), key=lambda item: (-item[1], item[0]))),
...
def build_release_certificate(
    *,
    ...
    certificate = {
        ...
        "admission_policy_path": str(admission_policy_path) if admission_policy_path is not None else None,
        ...
        "admission_policy_evaluation": _load_json(analysis / "admission_policy_evaluation.json"),
        "release_summary": _load_json(analysis / "release_summary.json"),
        "generator_feedback": _load_json(feedback_path),
        "visual_audit_manifest": _load_json(analysis / "visual_audit" / "visual_audit_manifest.json"),
        ...
        "profile_run_manifest": _load_json(analysis / "manifests" / "profile_run_manifest.json"),
        "required_artifacts": required,
        "artifact_hashes": _artifact_hashes(analysis, required),
    }
    certificate["certificate_status"] = "complete" if all(item["exists"] for item in required) else "incomplete"
    return certificate
...
def _variant_recommendations(
    ...
    for _, row in variants.iterrows():
        ...
        if status == "release" and not reasons:
            continue
        event_frame = events[events["variant_id"].astype(str) == variant_id] if not events.empty else pd.DataFrame()
        rows.append(
            {
                ...
                "needs_repair_event_share": _finite(row.get("needs_repair_event_share")),
                "reasons": reasons,
                "suggested_actions": _suggested_actions(reasons),
                "visual_examples": visual_by_variant.get(variant_id, [])[:3],
            }
        )
    return sorted(rows, key=lambda item: (_priority_rank(str(item["priority"])), str(item["variant_id"])))
...
def _suggested_actions(reasons: list[str]) -> list[str]:
    ...
    if any("support_leakage" in reason or "support" in reason for reason in reasons):
        actions.append("Review event support generation and label bounds; residual mass leaks outside declared support.")
    if any("boundary" in reason for reason in reasons):
        actions.append("Smooth anomaly transitions or expand support to include boundary effects.")
    if any("wrong_reason" in reason or "shortcut" in reason for reason in reasons):
        actions.append("Reduce marginal shortcuts and increase canonical relation witness strength.")
    if any("not_detected" in reason or "observable_not_detected" in reason for reason in reasons):
        actions.append("Increase anomaly duration/strength or lower noise for this profile.")
    if any("negative_control" in reason for reason in reasons):
        actions.append("Inspect negative-control triggers; detector evidence may not be specific to the intended event.")
    if any("calibration_low_resolution" in reason for reason in reasons):
        actions.append("Increase clean calibration instances before treating low-alpha conclusions as stable.")
    if any("repair" in reason or "max_needs_repair_share" in reason for reason in reasons):
        actions.append("Inspect generator parameters for this variant and rerun admission after repair.")
    if not actions:
...
def _visual_examples(frame: pd.DataFrame) -> dict[str, list[str]]:
    if frame.empty or "variant_id" not in frame.columns or "plot_path" not in frame.columns:
        return {}
...
def _priority(status: str, reasons: list[str]) -> str:
    if status == "reject":
        return "p0"
    if status == "needs_repair" or any("boundary" in reason or "support" in reason for reason in reasons):
        return "p1"
    ...
    return [
        {
            ...
            "sha256": _file_sha256(analysis / path) if (analysis / path).is_file() else None,
        }
        for path in paths
    ]


def _artifact_hashes(analysis: Path, artifacts: list[Mapping[str, Any]]) -> dict[str, str]:
    return {
