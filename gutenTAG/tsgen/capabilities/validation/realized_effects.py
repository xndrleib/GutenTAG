...
def _realized_row(
    ...
    if clean_seg.size and anom_seg.size:
        ...
        clean_scale = float(np.sqrt(np.mean(np.var(clean_seg, axis=0)))) if clean_seg.shape[0] else 1.0
        offset = float(np.mean(delta))
        row["realized_offset"] = finite_float(offset)
        row["standardized_offset"] = finite_float(offset / max(clean_scale, 1e-8))
        clean_var = np.var(clean_seg, axis=0) + 1e-8
        anom_var = np.var(anom_seg, axis=0) + 1e-8
        log_ratio = float(np.mean(np.log(anom_var / clean_var)))
        row["realized_log_var_ratio"] = finite_float(log_ratio)
        row["realized_mad_ratio"] = finite_float(_mad(anom_seg) / max(_mad(clean_seg), 1e-8))
        row["mean_drift_side_effect"] = finite_float(abs(offset))
        row["marginal_mean_shift_target"] = finite_float(float(np.mean(np.abs(np.mean(anom_seg, axis=0) - np.mean(clean_seg, axis=0)))))
        row["marginal_var_shift_target"] = finite_float(float(np.mean(np.abs(anom_var - clean_var))))
        row["shape_residual_ratio"] = finite_float(_shape_residual_ratio(clean_seg, anom_seg))
        row["template_distance_clean"] = finite_float(float(np.sqrt(np.mean(np.square(clean_seg - np.mean(clean_seg, axis=0, keepdims=True))))))
        row["template_distance_anomalous"] = finite_float(float(np.sqrt(np.mean(np.square(anom_seg - np.mean(anom_seg, axis=0, keepdims=True))))))
        spectral_clean = _spectral_peak(clean_seg)
        spectral_anom = _spectral_peak(anom_seg)
        row["spectral_peak_clean"] = finite_float(spectral_clean, default=math.nan)
        row["spectral_peak_anomalous"] = finite_float(spectral_anom, default=math.nan)
        row["spectral_peak_shift"] = finite_float(spectral_anom - spectral_clean, default=math.nan)
        row["band_energy_shift"] = finite_float(_band_energy(anom_seg) - _band_energy(clean_seg), default=math.nan)
    if len(context_channels) >= 2:
        ...
        clean_context = clean[context_idx[:, None], list(context_channels)] if context_idx.size else clean_event
        corr_context = _mean_signed_offdiag_corr(clean_context)
        corr_clean_event = _mean_signed_offdiag_corr(clean_event)
        corr_anom_event = _mean_signed_offdiag_corr(anom_event)
        abs_corr_context = _mean_abs_offdiag_corr(clean_context)
        abs_corr_clean_event = _mean_abs_offdiag_corr(clean_event)
        abs_corr_anom_event = _mean_abs_offdiag_corr(anom_event)
        ...
        row["corr_context_clean"] = finite_float(corr_context, default=math.nan)
        row["corr_event_clean"] = finite_float(corr_clean_event, default=math.nan)
        row["corr_event_anomalous"] = finite_float(corr_anom_event, default=math.nan)
        row["abs_corr_context_clean"] = finite_float(abs_corr_context, default=math.nan)
        row["abs_corr_event_clean"] = finite_float(abs_corr_clean_event, default=math.nan)
        row["abs_corr_event_anomalous"] = finite_float(abs_corr_anom_event, default=math.nan)
        row["realized_fisher_shift"] = finite_float(
            _mean_abs_offdiag_fisher_shift(clean_event, anom_event),
            default=math.nan,
        )
        row["target_vs_ensemble_corr_context"] = finite_float(mode_context, default=math.nan)
        row["target_vs_ensemble_corr_event"] = finite_float(mode_event, default=math.nan)
        row["pca_residual_ratio"] = finite_float(_pca_residual_ratio(anom_event, clean_event), default=math.nan)
        row["regression_residual_ratio"] = finite_float(_regression_residual_ratio(anom_event, clean_event), default=math.nan)
        row["clean_mode_agreement"] = finite_float(mode_context, default=math.nan)
        row["anomalous_mode_agreement"] = finite_float(mode_event, default=math.nan)
    return row
...
def _segment(series: np.ndarray, start: int, end: int, channels: tuple[int, ...]) -> np.ndarray:
    if not channels:
...
def _target_vs_context_corr(
    ...
    if values.shape[0] < 3 or values.shape[1] < 2:
        return math.nan
    target_channels = tuple(dict.fromkeys(group.primary_channels or group.intervention_channels))
    target_positions = [
        index
...
def _regression_residual_ratio(anom_event: np.ndarray, clean_event: np.ndarray) -> float:
    return _target_regression_residual(anom_event) / max(_target_regression_residual(clean_event), 1e-8)
