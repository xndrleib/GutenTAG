...
from ..pandas_typing import as_frame, frame_groupby, row_mapping
...
def compute_shortcut_audit(
    ...
    for instance in dataset.instances:
        for group in instance.event_groups:
            ...
            frame = (
                as_frame(grouped.get_group(uid))
                if grouped is not None and uid in grouped.groups
                else pd.DataFrame()
            )
...
def _shortcut_row(
    ...
    best_canonical_relation = _max_score(
        ...
        and _int_value(row.get("projection_size", 0)) >= 2,
    )
    best_univariate = _max_score(
        frame, lambda row: _int_value(row.get("projection_size", 0)) == 1
    )
    best_boundary = math.nan
    ratio = (
        best_canonical_relation / max(best_univariate, 1e-12)
        if math.isfinite(best_canonical_relation)
        else math.nan
    )
    ...
    shortcut_dominated = bool(
        relation_like
        and math.isfinite(ratio)
        and ratio < 0.75
        and best_univariate > 0.0
    )
    status = (
        "shortcut_dominated"
    ...
    return {
        ...
        "best_canonical_relation_score": finite_float(
            best_canonical_relation, default=math.nan
        ),
        "best_univariate_witness": _best_witness(
            frame, lambda row: _int_value(row.get("projection_size", 0)) == 1
        ),
...
def _is_relation_like(group: EventGroup) -> bool:
    ...
    return (
        scope in RELATION_SCOPES
        or "relation" in scope
        or "dependence" in group.constraint_tag
    )


def _max_score(
    frame: pd.DataFrame, predicate: Callable[[Mapping[str, object]], bool]
) -> float:
    if frame.empty:
        return math.nan
    values: list[float] = []
    for _, row in frame.iterrows():
        ...
        value = payload.get("distance_value")
        if predicate(payload) and bool(pd.notna(value)):
            values.append(finite_float(value, default=math.nan))
...
def _best_witness(
    ...
    for _, row in frame.iterrows():
        payload = row_mapping(row)
        value = payload.get("distance_value")
        if not predicate(payload) or bool(pd.isna(value)):
            continue
        score = finite_float(value, default=math.nan)
        if score > best_value:
            best_value = score
            best_name = str(payload.get("witness_family", ""))
    return best_name


def _best_family(frame: pd.DataFrame, witnesses: set[str]) -> float:
    return _max_score(
        frame, lambda row: str(row.get("witness_family", "")) in witnesses
    )
