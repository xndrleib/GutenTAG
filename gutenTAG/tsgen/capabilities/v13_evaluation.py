...
The maximum over ALL configured windows/channels/pairs is calibrated on whole,
independent normal trajectories from the same target system.
"""
...
def blind_scan(reference: np.ndarray, values: np.ndarray, *, windows: tuple[int, ...],
               stride: int, timestamps: np.ndarray | None = None) -> ScanResult:
    ...
    if not np.isfinite(reference).all() or np.isinf(values).any():
        raise ValueError("reference must be finite; query may contain NaNs, not infinities")
    ...
    for w in sorted(set(windows)):
        ...
        starts.extend(grid); ends.extend(s+w for s in grid)
    start, end = np.asarray(starts), np.asarray(ends)
    center = reference.mean(axis=0)
    scale = reference.std(axis=0); scale = np.maximum(scale, 1e-8)
    ...
    scores = np.maximum(np.max(np.abs(mean), axis=1), np.max(np.abs(np.log(var)), axis=1))
    scores = np.maximum(scores, 10*_window_mean(missing.astype(float), start, end).max(axis=1))
    rz = np.sign((reference-center)/scale); qz = np.sign(z)
    ...
    for i in range(0, len(pairs), 32):
        ...
        scores = np.maximum(scores, np.max(np.abs(qm-rm), axis=1))
    if timestamps is not None:
        t = np.asarray(timestamps, dtype=float)
        if t.shape != values.shape or not np.isfinite(t).all() or np.any(np.diff(t, axis=0) <= 0):
            raise ValueError("timestamps must be finite and strictly increasing per channel")
        jitter = np.r_[np.zeros((1, values.shape[1])), np.abs(np.diff(t, axis=0)-1)]
        scores = np.maximum(scores, _window_mean(jitter, start, end).max(axis=1))
    return ScanResult(start, end, scores)
...
def empirical_p(null: np.ndarray, observed: np.ndarray) -> np.ndarray:
    ...
    return (1+null.size-np.searchsorted(null, observed, side="left"))/(null.size+1)


def evaluate_benchmark(root: Path, output: Path, *, verify_hashes: bool = True) -> dict[str, Any]:
    ...
    for system in systems:
        ...
        for p in sorted((folder / "calibration").glob("*.npz")):
            with np.load(p) as f:
                null.append(blind_scan(reference, f["values"], windows=config.window_lengths, stride=config.stride).maximum)
        ...
        for query in system["queries"]:
            with np.load(folder / "queries" / (query["id"]+".npz")) as f:
                ...
                scan = blind_scan(reference, f["values"], windows=config.window_lengths,
                                  stride=config.stride, timestamps=f["timestamps"])
            predictions.append((query, scan, empirical_p(np.asarray(null), scan.scores)))
        for query, scan, pvalues in predictions:
            events = json.loads((folder / "oracle" / (query["id"]+".json")).read_text())
            event = events[0]
            ...
            if system["role"] == "relation_only" and not (event["relation_only"] or event["is_null"]):
                raise ValueError("relation_only corpus contains a marginal/sensor event")
            for alpha in config.alpha_grid:
                ...
                outside = ~endpoint_in_event if not event["is_null"] else np.ones(len(alerts), bool)
                false_alerts = alerts & outside & evaluable[scan.ends-1]
                rows.append({"system_id": system["system_id"], "law_id": system["law_id"],
                             "split": system["split"], "role": system["role"],
                             "genotype_id": query["genotype_id"], "query_id": query["id"],
                             "mechanism": query["mechanism"], "alpha": alpha,
                             "strength": event["strength"], "duration": hi-lo,
                             "source_length": hi-lo, "transition_length": margin,
                             "is_null": event["is_null"], "maximum": scan.maximum,
                             "scan_p": float(empirical_p(np.asarray(null), np.array([scan.maximum]))[0]),
                             "event_detected": bool(detected.any()),
                             "interior_detected": bool((alerts & interior).any() and not event["is_null"]),
                             "interior_evaluable": bool(interior.any()),
                             "latency": int(scan.ends[detected].min()-lo) if detected.any() else None,
                             "false_alert_windows": int(false_alerts.sum()),
                             "calibration_count": len(null),
                             "calibration_resolution_ok": 1/(len(null)+1) <= alpha,
                             "information_mode": "fully_blind_fixed_grid_reference_conditioned"})
    summary = {"schema_version": "synthgen.v13.evaluation.v1", "query_alpha_rows": len(rows),
               "independent_systems": len(systems),
               "calibration_resolution_ok": all(r["calibration_resolution_ok"] for r in rows),
               "scientific_admission": "not_automatic", "rows": rows}
