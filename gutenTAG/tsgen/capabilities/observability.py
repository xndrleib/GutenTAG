...
from .ontology import available_witnesses_for_projection, canonical_witnesses_for_anomaly
...
def compute_observability_profiles(
    ...
    for instance in dataset.instances:
        ...
        for group in instance.event_groups:
            ...
            summary_rows.append(_event_summary_row(instance, group, event_rows, protocol))
    observability = pd.DataFrame(rows)
    event_summary = pd.DataFrame(summary_rows)
    arity = _build_arity_profile(event_summary, protocol)
    return ObservabilityResult(observability=observability, arity=arity, event_summary=event_summary)
...
def _event_observability_rows(
    *,
    ...
    candidate_channels = tuple(sorted(set(group.group_channels) | set(group.context_channels) | set(group.intervention_channels)))
            ...
            rows.append(
                {
                    **_event_common(instance, group, uid),
                    ...
                    "support_concentration_l2": finite_float(scores.get("support_concentration_l2", float("nan")), default=float("nan")),
                    "canonical_witness": witness in canonical_witnesses_for_anomaly(group.anomaly_type),
                }
            )
    return rows
...
def _event_summary_row(
    ...
    for row in rows:
        ...
        if bool(row.get("canonical_witness", False)):
            canonical_values_by_size[size] = max(canonical_values_by_size.get(size, 0.0), value)
        support_value = row.get("support_concentration_l2")
        if support_value is not None and math.isfinite(float(support_value)):
            support_concentration_values.append(float(support_value))
    ...
    result = {
        **common,
        ...
        "support_concentration_l2_max": finite_float(max(support_concentration_values) if support_concentration_values else float("nan"), default=float("nan")),
    }
    for size in range(1, int(protocol.max_projection_size) + 1):
        result[f"D_s{size}"] = finite_float(cumulative.get(size, 0.0))
        result[f"D_canonical_s{size}"] = finite_float(canonical_cumulative.get(size, 0.0))
    return result


def _build_arity_profile(event_summary: pd.DataFrame, protocol: CapabilityProtocol) -> pd.DataFrame:
    rows: list[dict[str, object]] = []
    if event_summary.empty:
        return pd.DataFrame(rows)
    for _, record in event_summary.iterrows():
        d_values = {size: float(record.get(f"D_s{size}", 0.0)) for size in range(1, protocol.max_projection_size + 1)}
        canonical_d_values = {
            size: float(record.get(f"D_canonical_s{size}", 0.0))
        ...
        for delta in protocol.delta_grid:
            ...
            for size in range(1, protocol.max_projection_size + 1):
                if observed_arity is None and d_values[size] > float(delta):
                    observed_arity = size
                if canonical_observed_arity is None and canonical_d_values[size] > float(delta):
            ...
            rows.append(
                {
                    ...
                    "delta": float(delta),
                    "observed_arity": observed_arity if observed_arity is not None else math.nan,
                    "is_observable_at_delta": observed_arity is not None,
                    "canonical_observed_arity": canonical_observed_arity if canonical_observed_arity is not None else math.nan,
                    "canonical_is_observable_at_delta": canonical_observed_arity is not None,
                    "semantic_arity": _semantic_arity(record),
                    "D_at_max_projection": d_values[protocol.max_projection_size],
                    "D_canonical_at_max_projection": canonical_d_values[protocol.max_projection_size],
                    "arity_gap_s2_minus_s1": finite_float(d_values.get(2, d_values.get(1, 0.0)) - d_values.get(1, 0.0)),
...
def _event_common(instance: InstanceRecord, group: EventGroup, uid: str) -> dict[str, object]:
    return {
        "event_id": uid,
