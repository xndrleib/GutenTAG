...
def grouped_centroid_accuracy(x: np.ndarray, labels: np.ndarray,
    ...
    if x.ndim != 2 or len(labels) != len(x) or len(groups) != len(x) or not np.isfinite(x).all():
...
def paired_c2st(clean: np.ndarray, anomalous: np.ndarray,
    ...
    return grouped_centroid_accuracy(np.vstack([clean, anomalous]),
                                     np.r_[np.zeros(n), np.ones(n)], np.r_[group, group])
...
def cluster_energy_interval(clean: np.ndarray, anomalous: np.ndarray,
                            groups: np.ndarray, *, samples: int,
                            rng: np.random.Generator, block: int = 256) -> tuple[float, float]:
