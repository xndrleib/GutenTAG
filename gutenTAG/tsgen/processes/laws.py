...
def require_spd(value: np.ndarray, *, correlation: bool = False) -> np.ndarray:
    ...
    if not np.isfinite(x).all() or not np.allclose(x, x.T, atol=1e-12, rtol=0):
        raise ValueError("covariance must be finite and symmetric")
    if correlation and not np.allclose(np.diag(x), 1.0, atol=1e-12, rtol=0):
...
class ProcessLaw:
    ...
    def sample(self, length: int, seed: int) -> Realization:
        ...
        if self.family == "diagonal-ar":
            stationary = self.covariance / (
                1 - self.coefficients[:, None] * self.coefficients[None, :]
            )
...
def ar_filter(
    innovations: np.ndarray, coefficients: np.ndarray, initial: np.ndarray
) -> np.ndarray:
...
def sample_law(
    ...
    *,
    ...
    if family == "diagonal-ar":
        ...
        return ProcessLaw(
            family, mean, a, cov, np.empty((channels, 0)), np.ones(channels)
        )
    ...
    b = (
        np.eye(channels)
        if family == "independent-gp"
        else rng.normal(size=(channels, q))
    )
    if family == "gp-lmc":
    ...
    specs = tuple(
        KernelSpec(
            ...
            period=4 * length_scale,
            alpha=float(rng.uniform(0.5, 2)),
        )
        for _ in range(q)
    )
