...
SIGNAL_KINDS = ("mean", "amplitude", "noise-scale", "slope", "curvature", "bump",
                "impulse", "factor-loading")
...
class Intervention:
    ...
    def validate(self, length: int, channels: int) -> None:
        ...
        if len(set(self.channels)) != len(self.channels) or any(c < 0 or c >= channels for c in self.channels):
...
def apply_interventions(realization: Realization, events: list[Intervention],
                        *, seed: int) -> InterventionResult:
    ...
    for index, event in enumerate(events):
        ...
        if occupied[event.start:event.end].any():
            raise ValueError("v13 plans must not overlap; composition requires an explicit contract")
        ...
        if not active:
            ...
            if law.family != "diagonal-ar" or c < 2:
                raise ValueError("exact relation-only requires diagonal-ar and >=2 channels")
            sigma = law.covariance
            target = selected[0]
            if event.kind == "covariance-change":
                signs = np.ones(c); signs[target] = -1
                endpoint = sigma * np.outer(signs, signs)
            else:
                precision = np.linalg.inv(sigma)
                partner = (target+1) % c
                v = np.zeros(c); v[target] = 1; v[partner] = -1
            ...
            for weight in np.unique(g[event.start:event.end]):
                ...
                mixed = (1-weight*event.strength)*sigma + weight*event.strength*endpoint
                innovations[where] = realization.standard_noise[where] @ np.linalg.cholesky(mixed).T
            pure = True
            selected = list(range(c))
            extra = {"sigma_before": sigma.tolist(), "sigma_endpoint": endpoint.tolist(),
            ...
            for ch in selected:
                ...
                sensor = apply_sensor_artifact(
                    values[lo:hi, ch], kind=event.kind, severity=event.strength, rng=rng,
                    timestamps=times[lo:hi, ch],
                    previous_value=float(values[lo-1, ch]) if lo else float(values[0, ch]),
                    reference_center=float(law.mean[ch]), reference_scale=float(law.marginal_std[ch]),
                )
            ...
            if event.kind in ("slope", "curvature", "bump", "impulse"):
                shape[:] = 0
                local = {"slope": u, "curvature": u*u, "bump": np.sin(np.pi*u)**2,
                         "impulse": (np.arange(u.size) == u.size//2).astype(float)}[event.kind]
                shape[event.start:event.end] = local
            for ch in selected:
                if event.kind == "amplitude":
                    values[:, ch] += np.expm1(event.strength*g)*realization.signal[:, ch]
                elif event.kind == "noise-scale":
                    if law.family == "diagonal-ar":
                        innovations[:, ch] *= np.exp(event.strength*g)
                    else:
                        values[:, ch] += np.expm1(event.strength*g)*realization.noise[:, ch]
                else:
                    values[:, ch] += event.strength*shape*law.marginal_std[ch]
        if active:
            mask[event.start:event.end, selected] = True
        metadata.append({**vars(event), **extra, "event_id": f"{realization.realization_id}:event:{index}",
                         "law_id": law.law_id, "realization_id": realization.realization_id,
                         "is_null": not active, "relation_only": pure,
                         "requested_strength": event.strength, "effective_strength": event.strength})
    if law.family == "diagonal-ar":
        values += ar_filter(innovations, law.coefficients, realization.initial)-realization.signal
    effect = (~np.isclose(values, realization.values, rtol=0, atol=1e-12, equal_nan=True)
              | (times != np.arange(n)[:, None]))
    for event in metadata:
        lo, hi = event["start"], event["end"]
        delta = values[lo:hi]-realization.values[lo:hi]
        finite = delta[np.isfinite(delta)]
        event["realized_rms"] = float(np.sqrt(np.mean(finite**2))) if finite.size else None
    return InterventionResult(values, np.isfinite(values), times, mask, effect, evaluation, metadata)
