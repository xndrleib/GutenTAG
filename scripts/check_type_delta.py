...
def diagnostics(root: Path, destination: Path) -> Counter:
    result = subprocess.run(
        [sys.executable, "-m", "mypy", "gutenTAG", "tests", "--no-pretty", "--no-incremental"],
    ...
    lines = [re.sub(r":\d+(?::\d+)?: error:", ": error:", line)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="v13-type-base-") as temporary:
        ...
        subprocess.run(["git", "worktree", "add", "--detach", str(base), a.base_sha], cwd=root, check=True)
        try:
    ...
    report = {"base_sha": a.base_sha,
              "head_sha": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=root, text=True).strip(),
              "base_errors": sum(old.values()), "head_errors": sum(new.values()),
              "new_errors": added, "resolved_errors": resolved,
              "full_mypy_clean": not new, "incremental_gate_passed": not added}
