...
from gutenTAG.tsgen.ood_splits import (
    parameter_range_overlap,
...
@pytest.mark.parametrize(
    "kind", ["clipping", "quantization", "stuck-at", "dropout", "timestamp-jitter"]
)
def test_sensor_artifacts_are_deterministic(kind):
    values = np.linspace(-1.0, 1.0, 64)
    first = apply_sensor_artifact(
        values, kind=kind, rng=np.random.default_rng(7), severity=0.6
    )
    second = apply_sensor_artifact(
        values, kind=kind, rng=np.random.default_rng(7), severity=0.6
    )
...
def test_fingerprint_classifier_detects_obvious_nuisance_leakage():
    ...
    for label, length in [("mean", 10.0), ("variance", 100.0)]:
        for delta in range(8):
            rows.append(
                {
                    "source_length": length + delta * 0.01,
